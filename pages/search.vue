...
async function search(page = 1) {
  ...
  loading.value = true
  errorMessage.value = ''
  await router.replace({ query: { q: normalized, ...(includeDeleted.value ? { deleted: '1' } : {}) } })

  try {
    const response = await $fetch<{ items: SearchResult[]; pagination: Pagination }>('/api/search', {
      query: { q: normalized, includeDeleted: includeDeleted.value, page, pageSize: 25 },
    })
    items.value = response.items
    pagination.value = response.pagination
    searched.value = true
  } catch (error) {
    errorMessage.value = apiErrorMessage(error, 'Search is unavailable right now.')
  } finally {
    loading.value = false
  }
}
  ...
  <div class="content-page search-page">
    <header class="page-heading">
      <div>
        ...
        <h1>Search</h1>
        <p>Find phrases in article titles and content without loading full documents.</p>
      </div>
    </header>
    ...
    <form class="search-form" @submit.prevent="search(1)">
      ...
      <UButton type="submit" label="Search" icon="i-lucide-search" size="lg" :loading="loading" />
    </form>
    ...
    <div v-else-if="!searched" class="large-state">
      ...
      <span>Results include only a short excerpt; full content stays in article view.</span>
    </div>
    ...
    <section v-else class="results-list" aria-live="polite">
      ...
      <NuxtLink
        v-for="item in items"
        :key="item.id"
        :to="{ path: '/', query: { folder: item.folderId, article: item.id, view: 'editor' } }"
        class="search-result"
      >
        <span class="result-kicker">
          <UIcon name="i-lucide-folder" /> {{ item.folderName || item.folderId }}
          <UBadge v-if="isDeleted(item.deleted)" color="error" variant="subtle" size="sm">Deleted</UBadge>
        </span>
        <strong>{{ item.title || 'Untitled' }}</strong>
        <p>{{ item.excerpt }}</p>
        <span class="row-meta">
          {{ Number(item.count ?? 0).toLocaleString() }} words · Updated {{ formatWriterDate(item.updateTime) }}
        </span>
      </NuxtLink>

      <footer v-if="pagination.totalPages > 1" class="pagination-bar page-pagination">
        <UButton label="Previous" color="neutral" variant="outline" :disabled="pagination.page <= 1" @click="search(pagination.page - 1)" />
        <span>Page {{ pagination.page }} of {{ pagination.totalPages }}</span>
        <UButton label="Next" color="neutral" variant="outline" :disabled="pagination.page >= pagination.totalPages" @click="search(pagination.page + 1)" />
      </footer>
    </section>
  </div>
</template>
